...
import { LoanInputs, LoanCalculationResult, Currency } from '../types/loan';
...
const deserializeCalculation = (data: any): SavedCalculation => {
    return {
        id: data.id,
        ...
        results: {
            ...
            amortizationSchedule: data.results.amortizationSchedule.map((entry: any) => ({
                ...entry,
...
export const saveCalculation = async (
    ...
    try {
        ...
        const updatedHistory = [serializedNew, ...history.map((item) => ({
            id: item.id,
